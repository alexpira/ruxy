...
impl GatewayService {
	...
	fn mangle_request(cfg: &Config, action: &ConfigAction, req: Request<Incoming>, client_addr: &str, corr_id: &str) -> Result<Request<GatewayBody>, ServiceError> {
		let req = req.map(|v| {
	...
	async fn forward(cfg: &Config, action: &ConfigAction, req: Request<Incoming>, client_addr: &str, corr_id: &str) -> Result<Response<Incoming>, ServiceError> {
		let remote_request = Self::mangle_request(cfg, action, req, client_addr, corr_id)?;
